...
| Lever | Effect |
|-------|--------|
| `extraKnownMarketplaces` | Adds the private marketplace at `[org-name]/tt`. |
...
Before deploying, replace every bracketed token in `managed-settings.json`:

- `[company-name]` — slugified company name (used in marketplace name and OTel attributes)
- `[org-name]` — GitHub organization slug hosting this repo
- `[company-name].internal` — internal DNS for the OTel collector
- `[company-name].com` — company email domain
...
| Investigate an unauthorized plugin install | Logs are in the SIEM under `service.namespace=[company-name]`. Search by `tool=plugin_install`. See policies/03-incident-response-addendum-ai-tools.md. |
